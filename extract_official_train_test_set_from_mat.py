#!/usr/bin/env python
# -*- coding: utf-8 -*-
# copied from https://github.com/cleinc/bts with minor modifications
...
def convert_image(i, depth_raw, image):
    ...
    folder=out_folder / train_test

    img_depth = depth_raw * 1000.0
    img_depth_uint16 = img_depth.astype(np.uint16)
    depth_path=(folder / "depth")
    if not os.path.exists(depth_path):
        os.makedirs(depth_path)
    depth_path=depth_path / (str(i) +".png")
...
if __name__ == "__main__":
    ...
    for i, image in enumerate(images):
        ...
        convert_image(i, depth_raw[i, :, :].T, image.T)
